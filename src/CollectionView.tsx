 ...
 */
export function CollectionView({ cards, drawnCardIds, onClearCollection }: CollectionViewProps) {
    ...
    <section className="collection-panel" aria-label="生活卡图鉴">
      <div className="panel-heading">
        ...
        <div className="collection-actions">
          ...
          <button className="danger-button" type="button" onClick={onClearCollection} disabled={discovered_count === 0}>
            清空图鉴
          </button>
        </div>
      </div>
      ...
      <div className="collection-grid">
        {cards.map((card) => {
          ...
          return (
            <article className={`collection-card rarity-${card.rarity} ${is_drawn ? "" : "is-hidden"}`} key={card.id}>
              ...
              <div>
                ...
                <p>
                  {is_drawn
                    ? `${format_rarity_label(card.rarity)} · ${card.minutes} 分钟`
                    : `${format_rarity_label(card.rarity)}剪影 · 继续抽卡来解锁`}
                </p>
              </div>
            </article>
          );
